...
def make_rotamer_selector(residues, rotamers, energies, RRE, ESB, Ecutoff,\
                          fasta_iteration, experiment=None, previous=None):
    ...
    with open ('/gpfs/group/cdm8/default/IPRO_DDGun_linearized/modules/DDGun_output.out','r') as f:
        fr=f.readlines()
    score={}
    for line in fr[1:]:
        line=line.strip()
        ...
        if mutation=='A':
            ...
            mutation='THR'
        score[(position,mutation)]=S_DDG

    DLB= min(map(float,list(score.values())))
    print('DLB',DLB)
    DUB= max(map(float,list(score.values())))
    print('DUB',DUB)



    #adding HSD to score
    mykeys=list(score.keys())
    for i in range(len(mykeys)):
        x,y=mykeys[i]
        if (x,'HIS') in score.keys():
            score[(x,'HSD')]=score[(x,'HIS')]
    ...
    try:
        ...
        mutated={}
        for spot in spots:
            if len(rotamers[spot]) > maximum:
                maximum = len(rotamers[spot])
        cut_off_score=0
        for spot in spots:
           print('spot',spot)
           for k in range(maximum):
               DDG[(str(spot),k+1)]=cut_off_score
           for j,r in enumerate(rotamers[spot]):
               if not (str(spot),str(r.kind)) in fasta_iteration:
                   #print('({0} ,{1}, {2})'.format(str(spot),r.kind,str(j+1)))
                   mutated[(str(spot),j+1)]=1
               else:
                   mutated[(str(spot), j+1)]=0
          # print('({0} ,{1}, {2})'.format(str(spot),r.kind,str(j+1)))

        for spot in spots:
            if str(spot) in list_DP:
                for j,r in enumerate(rotamers[spot]):
                    print('spot_design',spot)
                    if (str(spot), r.kind) in score.keys():

                        print('a',score[(str(spot), r.kind)])
                        DDG[(str(spot),j+1)]=score[(str(spot), r.kind)]
...
"""
        for spot in spots:
            script += "\t\t\t" + str(spot) + "\n"
        script += "/\n\tr\trotamers per residue /1*" + str(maximum) + """/;
...
"""
        ...
        cutoff = 10000.0
        # First declare the E_rs (rotamer-constant portions) values
        for spot in spots:
            # Loop through the possible rotamers
            for i in range(maximum):
                # If this rotamer exists
                if i < len(energies[spot]):
                    # If the value is greater than a cutoff (large, but needed
                    # to keep GAMS in the feasible range). Do this for both
                    # energies
                    if energies[spot][i][0] > cutoff:
                        energies[spot][i][0] = cutoff
                    if energies[spot][i][1] > cutoff:
                        energies[spot][i][1] = cutoff
                    # Include the E_rs and E_rb values
                    script += 'E_rs("' + str(spot) + '","' + str(i+1) + '") = '
                    script += format(energies[spot][i][0], '.3f') + ";\n"
                    script += 'E_rb("' + str(spot) + '","' + str(i+1) + '") = '
                    script += format(energies[spot][i][1], '.3f') + ";\n"
        ...
        # now read in the Esb value
        script += 'E_sb = ' + format(ESB, '.3f') + ';\n'
        # and read in energy cutoff value
        script += 'Ecutoff = ' + format(Ecutoff, '.3f') + ';\n'
        ...
        for spot in spots:
            for i in range(len(rotamers[spot])):
                ddg=DDG[(str(spot),i+1)]
                script +='DDG("'+str(spot)+'","'+str(i+1) + '") = '
                script +=str(ddg) + ';\n'
                script +='mutated("'+str(spot)+'","'+str(i+1) + '") = '
                script +=str(mutated[(str(spot),i+1)]) + ';\n'
        script +="Scalar"+"\t"+"DUB"+"\t"+"/"+str(DUB)+"/;"+'\n'
        script +="Scalar"+"\t"+"DLB"+"\t"+"/"+str(DLB)+"/;"+'\n'
        script += """
VARIABLES
\tobj
\tDmax
...
z.up(i,r,j,s) = 1;\n
"""
        # finding the objective of the current group
        objective = residues.objective
        if objective == "eliminate":
            obj = "MAXIMIZING"
        if objective == "improve":
            obj = "MINIMIZING"
        # put in that objective into GAMS
        script += "SOLVE best_rotamers USING mip {} obj;\n".format(obj)
        script += """
...
def optimal_rotamer_selector(residues, rotamers, energies, RRE, ESB, Ecutoff,fasta_iteration,
                             experiment = None, previous = None):
    ...
    script, inputFile, outputFile = make_rotamer_selector(residues, rotamers, \
                                    energies, RRE, ESB, Ecutoff, fasta_iteration, experiment, previous)
    # Run GAMS
